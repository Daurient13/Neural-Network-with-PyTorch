...
Neural network also called _multilayer perceptrons_. Neutral networks are one of the most popular (if not the most popular) macine learning models out there. They are so useful that the field has its own name: _deep learning_. Deep learning has numerous applications in the most cutting-edge areas of machine learning, including image recognition, natural language processing, medicine, and self-driving cars. Neural network are meant to, in a broad sense of the word, mimic how the human brain operates. They can be very complex.
